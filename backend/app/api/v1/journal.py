...
from app.schemas import JournalEntryRequest, JournalEntryResponse, JournalListResponse
...
async def create_entry(entry: JournalEntryRequest, db: AsyncSession = Depends(get_db)):
    ...
    entry_id = str(uuid4())
    entry_date = entry.date or datetime.now(timezone.utc).isoformat()
...
async def search_entries(query: str, limit: int = 5):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Поиск не удался") from None
