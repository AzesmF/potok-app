"""
Health Check, Audit API и экспорт артефактов Kon-Matrix.
Стандарт TRA-L3: наблюдаемость и программный доступ к артефактам аудита.
"""
...
async def health_check():
    ...
    return {
        ...
        "kon_matrix_compliance": {
            "TRA-L3": "PASS" if is_chain_valid else "FAIL",
        },
    }
...
async def get_audit_log(limit: int = 50):
    ...
    return {
        "total": len(logs),
        "chain_valid": worm_logger.verify_chain(),
        "logs": logs,
    }
...
async def get_kon_matrix_passport():
    ...
    if not os.path.exists(bundle_path):
        raise HTTPException(
            status_code=404,
            detail="Audit bundle not found. Run tools/export-audit-bundle.py first.",
        )
...
async def get_sbom_file():
    ...
    if not os.path.exists(sbom_path):
        raise HTTPException(
            status_code=404,
            detail="SBOM not found. Run tools/generate-sbom.py first.",
        )
